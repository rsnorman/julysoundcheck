...
RSpec.describe SheetSync::Download::TweetReviewRowParser do
  describe '#parse' do
    let(:row) do
    ...
    let(:reply_tweet) { nil }
    let(:review_tweet) do
      double('Tweet', tweet_id: SecureRandom.uuid,
    ...
    subject do
      described_class.new(row, review_tweet: review_tweet,
    ...
    it 'returns user attribute' do
      expect(subject.parse).to include(user: user)
    end

    it 'returns tweet_status_id attribute' do
      expect(subject.parse).to include(twitter_status_id: review_tweet.tweet_id)
    end

    it 'returns tweet attribute' do
      expect(subject.parse).to include(tweet: review_tweet)
    end
    ...
    context 'with two-part review tweet' do
      let(:reply_tweet) { double('Tweet') }

      it 'returns tweet attribute with reply tweet' do
        expect(subject.parse).to include(tweet: reply_tweet)
      end
    end
