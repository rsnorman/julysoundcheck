Given(/^an unsynced tweet review exists on the spreadsheet$/) do
...
Given(/^an unsynced review exists on the spreadsheet$/) do
  user = FactoryGirl.create(:user, name: 'Alice Glass')
  @unsaved_review = FactoryGirl.build(:review, user: user)
  SheetSync::Uploader.new(worksheet: @worksheet).upload(@unsaved_review)
end
...
Then(/^I see the newly downloaded tweet review$/) do
...
Given(/^a synced tweet review exists on the spreadsheet$/) do
  user = FactoryGirl.create(:user, :twitter_user, name: 'Alice Glass')
  tweet = FactoryGirl.create(:tweet, tweet_id: '781681048220536832', user: user)
